...
-- ----------------------------
 ...
 CREATE TABLE `flink_task_app` (
  ...
  `update_time` datetime DEFAULT NULL COMMENT '更新时间',
  PRIMARY KEY (`id`),
  UNIQUE KEY `idx_project_flow_task_execution_appId` (`project_name`,`flow_name`,`task_name`,`execution_time`,`application_id`)
) ENGINE=InnoDB AUTO_INCREMENT=36 DEFAULT CHARSET=utf8 COMMENT='实时任务application表'

 CREATE TABLE `flink_task_diagnosis` (
  `id` int(11) NOT NULL AUTO_INCREMENT COMMENT '实时任务诊断结果id',
  `flink_task_app_id` int(11) DEFAULT NULL,
  `username` varchar(64) DEFAULT NULL COMMENT '用户名',
  `user_id` int(11) DEFAULT NULL COMMENT '用户记录id',
  `project_name` varchar(64) DEFAULT NULL COMMENT '项目名',
  `project_id` int(11) DEFAULT NULL COMMENT '项目id',
  ...
  `running_containers` int(11) DEFAULT NULL COMMENT 'yarn获取的总共分配容器',
  `engine_type` varchar(64) DEFAULT NULL COMMENT '执行引擎',
  `execution_time` datetime DEFAULT NULL COMMENT '执行周期',
  `duration` double DEFAULT NULL COMMENT '运行耗时',
  `start_time` datetime DEFAULT NULL COMMENT '开始时间',
  `end_time` datetime DEFAULT NULL COMMENT '结束时间',
  `vcore_seconds` float DEFAULT NULL COMMENT 'cpu消耗(vcore-seconds)',
  ...
  `diagnosis` varchar(255) COMMENT 'yarn诊断信息',
  `parallel` int(11) DEFAULT NULL COMMENT 'flink 并行度',
  `tm_slot` int(11) DEFAULT NULL COMMENT 'flink slot',
  `tm_core` int(11) DEFAULT NULL COMMENT 'flink tm core',
  `tm_mem` int(11) DEFAULT NULL COMMENT 'flink tm_mem',
  `jm_mem` int(11) DEFAULT NULL COMMENT 'flink jm_mem',
  `tm_num` int(11) DEFAULT NULL COMMENT 'flink tm_num',
  `job_name` varchar(255) COMMENT 'job name',
  `diagnosis_end_time` timestamp NOT NULL COMMENT '诊断结束时间',
  `diagnosis_start_time` timestamp NULL DEFAULT NULL COMMENT '诊断起始时间',
  `diagnosis_resource_type` varchar(255) DEFAULT NULL COMMENT '资源诊断类型,[0扩容cpu,1扩容mem,2缩减cpu,3缩减mem,4运行异常]',
  `diagnosis_from` int(11) DEFAULT NULL COMMENT '诊断来源0凌晨定时任务,1任务上线后诊断,2即时诊断',
  `diagnosis_parallel` int(11) DEFAULT NULL COMMENT '建议并行度',
  `diagnosis_jm_mem_size` int(11) DEFAULT NULL COMMENT '建议job manager 内存大小单位MB',
  `diagnosis_tm_mem_size` int(11) DEFAULT NULL COMMENT '建议task manager 内存大小单位MB',
  `diagnosis_tm_slot_num` int(11) DEFAULT NULL COMMENT '建议tm的slot数量',
  `diagnosis_tm_core_num` int(11) DEFAULT NULL COMMENT '建议tm的core数量',
  `diagnosis_tm_num` int(11) DEFAULT NULL COMMENT '建议tm数量',
  `diagnosis_types` varchar(255) DEFAULT NULL COMMENT '诊断问题类型拼接字符串[0][1][2][3]',
  `process_state` varchar(200) DEFAULT NULL COMMENT '处理状态(processing, success, failed)',
  `create_time` datetime DEFAULT NULL COMMENT '创建时间',
  `update_time` datetime DEFAULT NULL COMMENT '更新时间',
  PRIMARY KEY (`id`),
  KEY `idx_diagnosisTime_diagnosisType` (`diagnosis_end_time`,`diagnosis_types`)
) ENGINE=InnoDB AUTO_INCREMENT=20184 DEFAULT CHARSET=utf8 COMMENT='实时任务诊断表'
