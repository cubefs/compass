...
const getLineChart = async () => {
  ...
  data = res[0]
  console.log(data)
  cpuData = lineTab === 'first' ? data.cpuTrend : data.jobNumberTrend
  memData = data.memoryTrend
}
const option = {
  title: {
    text: '',
...
const handleClick = (val) => {
  cpuData = val.paneName === 'first' ? data.cpuTrend : cpuData = data.jobNumberTrend
}
onMounted(() => {
  getLineChart()
  init()
...
<template>
  <el-tabs v-model="lineTab" @tab-click="handleClick">
    <div m-b-10 flex="~">
      <div style="width:62%" />
      <el-date-picker v-model="time" type="daterange" range-separator="-" start-placeholder="开始时间" end-placeholder="结束时间"
