...
public class DiagnosisServiceImpl implements DiagnosisService {
    ...
    private RealtimeTaskDiagnosisRuleAdvice buildAdvice(EDiagnosisRule t, RealtimeTaskDiagnosis realtimeTaskDiagnosis, String desc) {
        RealtimeTaskDiagnosisRuleAdvice realtimeTaskDiagnosisRuleAdvice = new RealtimeTaskDiagnosisRuleAdvice();
        realtimeTaskDiagnosisRuleAdvice.setHasAdvice((short) EDiagnosisRuleHasAdvice.HAS_ADVICE.getCode());
        realtimeTaskDiagnosisRuleAdvice.setRealtimeTaskDiagnosisId(realtimeTaskDiagnosis.getId());
        realtimeTaskDiagnosisRuleAdvice.setRuleType(t.getCode());
        realtimeTaskDiagnosisRuleAdvice.setRuleName(t.getName());
        realtimeTaskDiagnosisRuleAdvice.setDescription(desc);
        realtimeTaskDiagnosisRuleAdvice.setCreateTime(new Date());
        realtimeTaskDiagnosisRuleAdvice.setUpdateTime(new Date());
        return realtimeTaskDiagnosisRuleAdvice;
    }
    ...
    public RealtimeTaskDiagnosis diagnosisApp(RealtimeTaskApp realtimeTaskApp, long start, long end,
        ...
        try {
            ...
            YarnApp app = realtimeMetaService.requestYarnApp(appId);
            if (app == null) {
            ...
            if (
                    app.getState().equalsIgnoreCase(EYarnApplicationState.FINISHED.getDesc()) ||
                            app.getState().equalsIgnoreCase(EYarnApplicationState.FAILED.getDesc()) ||
                            app.getState().equalsIgnoreCase(EYarnApplicationState.KILLED.getDesc())
            ) {
                realtimeTaskApp.setTaskState(ERealtimeTaskAppState.FINISHED.getDesc());
                realtimeTaskAppMapper.updateByPrimaryKey(realtimeTaskApp);
                log.info(" app 已经停止 {}", appId);
                return null;
            }

            if (!app.getState().equalsIgnoreCase(EYarnApplicationState.RUNNING.name())) {
                log.info(" app state not running{}", appId);
                return null;
            }
            // 从flink ui config 获取 jobName,运行配置参数,如果访问不到，说明作业有问题
            List<JobManagerConfigItem> flinkConfigItems = realtimeMetaService.reqFlinkConfig(app);
            if (flinkConfigItems == null) {
                log.info("flink ui无法访问 {}", appId);
                return null;
            }
            String jobId = realtimeMetaService.getJobId(app);
            List<String> tmIds = realtimeMetaService.getTmIds(app);

            // 通过flink job manager的配置更新元数据
            realtimeMetaService.fillFlinkMetaWithFlinkConfigOnYarn(realtimeTaskApp, flinkConfigItems, jobId);
            // 更新元数据
            ...
            DiagnosisContext context = new DiagnosisContext(rcJobDiagnosis, start, end, realtimeDiagnosisMetricsServiceImpl, from);
            context.getMessages().put(EDiagnosisParam.JobId, jobId);
            context.getMessages().put(EDiagnosisParam.TmIds, tmIds);
            // 执行诊断
            ...
            realtimeTaskDiagnosisMapper.insertSelective(realtimeTaskDiagnosis);
            ...
            for (RcJobDiagnosisAdvice advice : advices) {
                ...
                realtimeTaskDiagnosisRuleAdvice.setCreateTime(new Date());
                realtimeTaskDiagnosisRuleAdvice.setUpdateTime(new Date());
                realtimeTaskDiagnosisRuleAdviceMapper.insertSelective(realtimeTaskDiagnosisRuleAdvice);
    ...
    @Override
    public void diagnosisAllApp(long start, long end, EDiagnosisFrom from) {
        RealtimeTaskAppExample realtimeTaskAppExample = new RealtimeTaskAppExample();
        realtimeTaskAppExample.createCriteria()
                .andTaskStateEqualTo(ERealtimeTaskAppState.RUNNING.getDesc())
        ;
        long count = realtimeTaskAppMapper.countByExample(realtimeTaskAppExample);
        int pageSize = 100;
    ...
    private Boolean checkNeedDiagnosisHourly(RealtimeTaskApp job, long start, long end) {
        ...
        String jobId = realtimeMetaService.getJobId(job.getFlinkTrackUrl());
        String jobUpTime = realtimeDiagnosisMetricsServiceImpl.addLabel(JOB_UP_TIME, "job_id", jobId);
        List<MetricResult.DataResult> jobUpTimeMetrics = realtimeDiagnosisMetricsServiceImpl
     ...
     */
    @Override
    public void diagnosisAppHourly(long start, long end, EDiagnosisFrom from) {
        RealtimeTaskAppExample realtimeTaskAppExample = new RealtimeTaskAppExample();
        realtimeTaskAppExample.createCriteria()
                .andTaskStateEqualTo(ERealtimeTaskAppState.RUNNING.getDesc())
        ;
    ...
    private void fillRealtimeTaskDiagnosisWithDiagnosisContext(RealtimeTaskDiagnosis realtimeTaskDiagnosis, DiagnosisContext context) {
        ...
        if (realtimeTaskDiagnosis.getTmNum() != null && realtimeTaskDiagnosis.getTmCore() != null) {
            ...
            // 判断资源变化
            if (newCore > preCore) {
                addDiagnosisResourceTypes(realtimeTaskDiagnosis, EDiagnosisResourceType.GROW_CPU.getCode());
            } else if (newCore < preCore) {
                addDiagnosisResourceTypes(realtimeTaskDiagnosis, EDiagnosisResourceType.CUT_CPU.getCode());
            }
        }
        // 资源调优类型
        if (realtimeTaskDiagnosis.getTmNum() != null && realtimeTaskDiagnosis.getTmMem() != null) {
            ...
            // 判断资源变化
            if (newMem > preMem) {
                addDiagnosisResourceTypes(realtimeTaskDiagnosis, EDiagnosisResourceType.GROW_MEM.getCode());
            } else if (newMem < preMem) {
                addDiagnosisResourceTypes(realtimeTaskDiagnosis, EDiagnosisResourceType.CUT_MEM.getCode());
            }
        }
        ...
        for (RcJobDiagnosisAdvice rcJobDiagnosisAdvice : advices) {
            if (rcJobDiagnosisAdvice.getHasAdvice()) {
                if (rcJobDiagnosisAdvice.getAdviceType().getCode() == EDiagnosisRuleType.RuntimeExceptionRule.getCode()) {
                    addDiagnosisResourceTypes(realtimeTaskDiagnosis, EDiagnosisResourceType.EXCEPTION.getCode());
                }
