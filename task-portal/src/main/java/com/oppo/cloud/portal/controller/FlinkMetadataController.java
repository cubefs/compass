...
/**
 * Flink HTTP API report metadata
 */
@Slf4j
@Controller
public class FlinkMetadataController {
    ...
    @ApiOperation(value = "report metadata")
    public CommonStatus<?> saveRealtimeTaskApp(@RequestBody FlinkTaskApp flinkTaskApp) {
        ...
        if (flinkTaskApps == null || flinkTaskApps.size() == 0) {
            ...
            log.error("task metadata should be unique, appId: {}", flinkTaskApp.getApplicationId());
            return CommonStatus.failed("failed to report metadata, record should be unique, appId: " + flinkTaskApp.getApplicationId());
        }
    }
}
